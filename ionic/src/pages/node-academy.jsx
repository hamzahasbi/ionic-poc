...
const AcademyDetail = ({ node }) => {
  const disqusIdentifier = node.field_nid
    ? `node/${node.field_nid}`
    : `node/${node.drupal_internal__nid}`;
  const videoURL = getNestedValue('field_vactory_youtube_videos', node);
  const avatarURL = getNestedValue(
    'internal_user.field_vactory_instructor.picture._default',
    node,
  );
  const documentURL = getNestedValue(
    'field_vactory_file.uri.value._default',
    node,
  );

  const authorFirstName = getNestedValue(
    'internal_user.field_vactory_instructor.first_name',
    node,
  );
  const authorLastName = getNestedValue(
    'internal_user.field_vactory_instructor.last_name',
    node,
  );
  ...
  const author_about = getNestedValue(
    'internal_user.field_vactory_instructor.about.0.value',
    node,
  );
      ...
      <IonRow>
        <IonCol size="12" className="ion-align-items-center">
          <IonLabel className="detail-label">
            ...
            <p
              color="dark"
              dangerouslySetInnerHTML={{ __html: sanitizeHtml(author_about) }}
            />
